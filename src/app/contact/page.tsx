      ...
      <header className={`${isDark ? 'bg-[#222]' : 'bg-white'} shadow-sm sticky top-0 z-40 w-full block lg:hidden`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`relative flex items-center py-2 border-b ${isDark ? 'border-[#333]' : 'border-gray-100'} min-h-[48px]`}>
            ...
            <h1 className="mx-auto text-xl font-bold truncate max-w-xs text-center w-full" style={{ color: isDark ? '#fff' : '#222' }}>{language === 'ar' ? 'اتصل بنا' : 'Contact'}</h1>
          </div>
        </div>
      </header>
      
      {/* Body: logo at top, then contact block, responsive */}
